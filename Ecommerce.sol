...
contract Ecommerce {
    ...
    function delivery(uint _productId) public {
        require(products[_productId].buyer == msg.sender, "Only buyer cna confirm it");
        products[_productId].delivered = true;
        products[_productId].seller.transfer(products[_productId].price);
        emit delivered(_productId);
    }

    function cancelOrder(uint _productId) public {
        require(products[_productId].buyer == msg.sender, "You didn't order this item");
        products[_productId].buyer = address(0);
    }
